...
| post-Phase-5 | 2026-05-15 | (머지 후 채움) | typescript / security (java 생략 — backend 변경 0) | CompareLoader 환자-슬롯 cap 명시 + carry-over, `countValidIdSegments` 신설로 too-many 오분류 차단, OverlayChart series useMemo, dead `label` prop 제거, compareIds DoS 하드닝(segment cap) | `IMPL_SPEC.md §8.9` |
